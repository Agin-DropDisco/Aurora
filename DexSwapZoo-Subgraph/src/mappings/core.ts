...
import { updatePairDayData, updateTokenDayData, updateDexsDayData, updatePairHourData } from './dayUpdates'
import {
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = DexsFactory.load(getFactoryAddress())
...
export function handleSync(event: Sync): void {
  ...
  dexs.totalLiquidityNativeCurrency = dexs.totalLiquidityNativeCurrency.minus(
  ...
  dexs.totalLiquidityNativeCurrency = dexs.totalLiquidityNativeCurrency.plus(trackedLiquidityNativeCurrency)
  dexs.totalLiquidityUSD = dexs.totalLiquidityNativeCurrency.times(bundle.nativeCurrencyPrice)
...
export function handleMint(event: Mint): void {
  ...
  dexs.txCount = dexs.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let dexs = DexsFactory.load(getFactoryAddress())
  dexs.totalVolumeUSD = dexs.totalVolumeUSD.plus(trackedAmountUSD)
  dexs.totalVolumeNativeCurrency = dexs.totalVolumeNativeCurrency.plus(trackedAmountNativeCurrency)
  dexs.untrackedVolumeUSD = dexs.untrackedVolumeUSD.plus(derivedAmountUSD)
  dexs.txCount = dexs.txCount.plus(ONE_BI)
  ...
  let dexsDayData = updateDexsDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  dexsDayData.dailyVolumeUSD = dexsDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  dexsDayData.dailyVolumeNativeCurrency = dexsDayData.dailyVolumeNativeCurrency.plus(trackedAmountNativeCurrency)
  dexsDayData.dailyVolumeUntracked = dexsDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  dexsDayData.save()
