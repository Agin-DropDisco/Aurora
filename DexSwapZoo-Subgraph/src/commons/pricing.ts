...
export function getMinimumUsdThresholdForNewPairs(): BigDecimal {
  ...
  if (network == 'aurora') return BigDecimal.fromString('0.1')
  if (network == 'mumbai') return BigDecimal.fromString('0.1')
...
export function getMinimumLiquidityThresholdNativeCurrency(): BigDecimal {
  ...
  if (network == 'aurora') return BigDecimal.fromString('0.01')
  if (network == 'mumbai') return BigDecimal.fromString('0.01')
  log.warning('no minimum ust threshold for new pairs for unsupported network {}', [network])
  return ZERO_BD
}
